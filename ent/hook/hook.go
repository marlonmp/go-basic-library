...
// The BookLoanFunc type is an adapter to allow the use of ordinary
// function as BookLoan mutator.
type BookLoanFunc func(context.Context, *ent.BookLoanMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BookLoanFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BookLoanMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BookLoanMutation", m)
	}
	return f(ctx, mv)
}
