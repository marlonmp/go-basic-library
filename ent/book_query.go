...
func (bq *BookQuery) QueryOnLoans() *BookLoanQuery {
	query := &BookLoanQuery{config: bq.config}
...
func (bq *BookQuery) Clone() *BookQuery {
	...
	return &BookQuery{
		...
		withOnLoans: bq.withOnLoans.Clone(),
...
// WithOnLoans tells the query-builder to eager-load the nodes that are connected to
// the "on_loans" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookQuery) WithOnLoans(opts ...func(*BookLoanQuery)) *BookQuery {
	query := &BookLoanQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withOnLoans = query
	return bq
}
...
func (bq *BookQuery) sqlAll(ctx context.Context) ([]*Book, error) {
	...
	if query := bq.withOnLoans; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Book)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.OnLoans = []*BookLoan{}
		}
		query.withFKs = true
		query.Where(predicate.BookLoan(func(s *sql.Selector) {
			s.Where(sql.InValues(book.OnLoansColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.book_on_loans
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "book_on_loans" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "book_on_loans" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OnLoans = append(node.Edges.OnLoans, n)
		}
	}
